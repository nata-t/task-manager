...
import { AlertTriangle, CheckCircle2, Clock, FolderKanban, Info, ListTodo, TrendingUp, Users } from "lucide-react";
...
export function InfoTab({ workspace }: WorkspaceInfoTabProps) {
  ...
  const allTasks: { id: string, title: string, description: string | null, status: string, project_name: string }[] = [];
  ...
  return (
    <div>
      <div className="flex flex-col gap-1 mb-6">
        <h3 className="text-lg font-semibold text-foreground tracking-tight">Workspace Overview</h3>
        <p className="text-sm text-muted-foreground">Quick access to pressing tasks and summarized analytics.</p>
      </div>
       <div className="grid gap-4 md:grid-cols-2">
                ...
                {sortedTasksForOverview.slice(0, 6).map((task, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start gap-4">
                      <span className="font-semibold text-sm line-clamp-1 leading-tight">{task.title}</span>
                    </div>
                    {task.description && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Detailed metrics and insights for {workspace.name}.</CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            {overdueTasks > 0 ? (
              <div className="flex items-center gap-3 rounded-lg border border-destructive/20 bg-destructive/10 p-3 text-destructive">
                ...
                <p className="text-sm font-medium">You have {overdueTasks} overdue task{overdueTasks !== 1 && "s"}</p>
              </div>
            ) : (
              <div className="flex items-center gap-3 rounded-lg border border-primary/20 bg-primary/10 p-3 text-primary">
                <CheckCircle2 className="h-4 w-4 shrink-0" />
                <p className="text-sm font-medium">No overdue tasks. You are all caught up!</p>
              </div>
            )} 
            ...
            <div className="flex items-center gap-3 rounded-lg border bg-card p-3">
              ...
              <p className="text-sm text-foreground">
                Most active project: <span className="font-semibold">{mostActiveProject.name}</span>
              </p>
            </div>
          </CardContent>
        </Card>
