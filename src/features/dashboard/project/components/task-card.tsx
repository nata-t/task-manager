...
import { CalendarIcon, User, GripVertical, Trash2, CheckCircle2, Circle, Clock } from "lucide-react";
...
export function TaskCard({ 
  ...
  const handleSaveDesc = () => {
    if (editDesc.trim() !== (task.description || "")) {
      updateTask.mutate({ id: task.id, projectId, description: editDesc.trim() });
    } else {
  ...
  const StatusIcon = () => {
    if (task.status === "done") return <CheckCircle2 className="h-4 w-4 text-green-500" />;
    if (task.status === "in_progress") return <Clock className="h-4 w-4 text-yellow-500" />;
  ...
  return (
    <Card 
      ...
      className={cn(
        ...
        isDragging && "shadow-lg scale-100 z-10 rotate-1 ring-1 ring-primary/20",
    ...
    >
          ...
          {isEditingDesc ? (
             <Textarea
               ...
               onKeyDown={(e) => {
                 ...
                 if (e.key === "Escape") { setEditDesc(task.description || ""); setIsEditingDesc(false); }
               }}
            ...
            <p 
              className={cn(
                ...
                !task.description ? "italic text-muted-foreground/60" : "text-muted-foreground"
              )}
      ...
      <div className={cn(
        ...
        isList ? "sm:ml-auto ml-0 mt-2 sm:mt-0 items-start self-start" : "ml-6 mt-1"
      )}>
          ...
          <Select value={task.status} onValueChange={handleStatusChange}>
            ...
            <SelectContent>
              <SelectItem value="todo">
                <div className="flex items-center gap-1.5"><Circle className="h-3 w-3 text-muted-foreground" /> Todo</div>
              </SelectItem>
              <SelectItem value="in_progress">
                <div className="flex items-center gap-1.5"><Clock className="h-3 w-3 text-yellow-500" /> In Progress</div>
              </SelectItem>
              <SelectItem value="done">
                <div className="flex items-center gap-1.5"><CheckCircle2 className="h-3 w-3 text-green-500" /> Done</div>
              </SelectItem>
            </SelectContent>
          </Select>
        )}
        ...
        <Select
          ...
          onValueChange={(val) => {
            updateTask.mutate({ id: task.id, projectId, assignee_id: val === "unassigned" ? null : val });
          }}
        >
            ...
            <SelectValue placeholder="Unassigned">
              {task.profiles ? (
                <span className="flex items-center gap-1.5 font-medium">
                  ...
                  <span className="truncate max-w-[80px]">{task.profiles.full_name}</span>
                </span>
              ) : (
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                ...
                isOverdue && "text-destructive border-destructive/30 bg-destructive/5 hover:bg-destructive/10 hover:text-destructive"
              )}
            >
          ...
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
              ...
              onSelect={(date) => {
                ...
                updateTask.mutate({ id: task.id, projectId, due_date: dateStr });
